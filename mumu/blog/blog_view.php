    ...
    <td width="728" valign="top" >
     ...
     ?>
	   ...
	   <table width="100%" border="0" cellspacing="1" cellpadding="2">
        ...
        <tr> 
         <td class=line-n height="30" valign="top" bgcolor="#EEECCC"><b>블로그 정보</b> </td>
		 <td class=line-n height="30" valign="top" align=right bgcolor="#EEECCC"><b><a href="blog_brd_tlist.php?b_id=<?=$b_id?>">전체보기</a></b> </td>
       </tr>
	   ...
	   <tr>
		...
		?>
		    <table width="99%" border="0" cellspacing="0" cellpadding="0" class="line1">
			  ...
			  <input type='hidden' name='ret_url' value='blog_view.php'>
			  </form>
		   </table>
		 <? 
			 }
        ...
        <tr> 
         <td colspan="2" valign="middle" align=center bgcolor="#e1e1e1">
           <table width="99%" border="0" cellspacing="1" cellpadding="5" bgcolor="#CCCEEE">
		     <tr>
			    <td width="100" align="center" bgcolor="#FFFF99">이전글 : </td>
			    <td style="padding:5 0 7 5px" bgcolor="#FFFFFF">
	   <?
	      $p_qry = "select a.*,b.brd_title,b.brd_pow_2
		               from $blog_t a , blog_brd_list b 
		               where a.brd_list_fk = b.num And
					         a.num < '$brd_num' And
					         b.brd_pow_1 >= $auth_key 
							 order by a.num desc limit 0,1";
          $p_res = mysql_query($p_qry,$connect);
          $p_rows = mysql_fetch_array($p_res); 
		  if(!$p_rows){
			 echo"이전글이 존재하지 않습니다.";
		  }
		  else{
	   ?>
	        <a href="blog_view.php?b_id=<?=$b_id?>&brd_num=<?=$p_rows[num]?>"><?=$p_rows[title]?></a> ( <?=$p_rows[brd_title]?> ) 
	   <?
		  }
	   ?>
				</td>
			 ...
			 <tr>
			    <td width="100" align="center" bgcolor="#FFFF99">다음글 : </td>
			    <td style="padding:5 0 7 5px" bgcolor="#FFFFFF">
	   <?
	      $n_qry = "select a.*,b.brd_title,b.brd_pow_2
		               from $blog_t a , blog_brd_list b 
		               where a.brd_list_fk = b.num And
					         a.num > '$brd_num' And
					         b.brd_pow_1 >= $auth_key 
							 order by a.num desc limit 0,1";
          $n_res = mysql_query($n_qry,$connect);
          $n_rows = mysql_fetch_array($n_res); 
		  if(!$n_rows){
			 echo"다음글이 존재하지 않습니다.";
		  }
		  else{
	   ?>
	        <a href="blog_view.php?b_id=<?=$b_id?>&brd_num=<?=$n_rows[num]?>"><?=$n_rows[title]?></a> ( <?=$n_rows[brd_title]?> ) 
	   <?
		  }
	   ?>
				</td>
