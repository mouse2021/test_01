...
<table style="border-width:1; border-style:solid;" border="0" cellpadding="0" cellspacing="0" width="938">
  <tr>
    ...
    <td width="728" valign="top" >
     ...
     ?>
	   ...
	   <?
	    if($b_nm){
		  $qry3 = "select * from blog_brd_list where num='$b_nm' ";
          $res3  = mysql_query($qry3,$connect);
          $rows3 = mysql_fetch_array($res3);
		  $b_nm_qry = " And brd_list_fk='$b_nm' ";
		  echo"( $rows3[brd_title] )";
	    }
	   ?>
		 </td>
       </tr>
  ...
  ?>
		   ...
		   <table border=0 width="98%" align=center >
			 <tr bgcolor="#e1e1e1">
			   ...
			   <td width="30%" align=center>
				...
				<input type="button" name="삭제" value="삭제" onclick="location='blog_write_manager.php?b_id=<?=$b_id?>&md=delete&del_num=<?=$mrows[num]?>'" class="InputStyle">
				<?
				 }
				?>
			   </td>
	   ...
	   <tr>
	     <td colspan="2" valign="middle" align=center bgcolor="#99CCFF">
		   <table border=0 width="98%" align=center >
			 <tr>
			   <td>
			     댓글 : <?=number_format($mtot2)?> 개 (<a href="<?=$PHP_SELF?>?b_id=<?=$b_id?>&b_nm=<?=$b_nm?>&brd_num=<?=$mrows[num]?>&md1=view">보기</a>)
			   </td>
			 </tr>
			</table>
		  </td>
   ...
   ?>
	   <tr>
		 <td align=left colspan="2">
		    <table width="99%" border="0" cellspacing="1" cellpadding="5" bgcolor="#CCCEEE">
			 ...
			 ?>		  
			      <a href='blog_memo_manager.php?b_id=<?=$b_id?>&b_nm=<?=$b_nm?>&brd_num=<?=$mrows[num]?>&md=delete&md1=view&del_num=<?=$m_ksh[cnum]?>&ret_url=blog_list.php'>X</a>
		      <?		 
				 }
			   }
			  ?>
	...
	?>
			<table width="99%" border="0" cellspacing="0" cellpadding="0" class="line1">
			  ...
			  <input type='hidden' name='b_nm' value='<?=$b_nm?>'>
			  <input type='hidden' name='ret_url' value='blog_list.php'>
			  </form>
		   </table>
     <? 
		 }
   ...
   ?>
    ...
    <?
	//페이지 나누기
	$url = $PHP_SELF."?b_id=".$b_id."&b_nm=".$b_nm; 
